package com.wdxxl.lucene.facet;
...
public class FacetDemo {
	...
	public static void main(String[] args) throws IOException {
		FacetDemo demo = new FacetDemo();
		demo.index();
		demo.facetsWithSearch();
	}
	...
	private void facetsWithSearch() throws IOException {
		...
		// 如果已经进入小米手机，但是还想看到其他牌子(华为)的手机数目，就用到了sideways
		System.out.println("-----小米手机 Drill Sideways-----");
		DrillSideways ds = new DrillSideways(searcher, config, taxoReader);
		DrillDownQuery drillDownQuery1 = new DrillDownQuery(config, query);
		drillDownQuery1.add("brand", "小米");
		DrillSidewaysResult result1 = ds.search(drillDownQuery1, 10);
		results = result1.facets.getAllDims(10);
		for (FacetResult tmp : results) {
			System.out.println(tmp);
		}
