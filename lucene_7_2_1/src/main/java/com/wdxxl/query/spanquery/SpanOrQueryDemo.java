...
public class SpanOrQueryDemo {
	...
	public static void main(String[] args) throws IOException {
		SpanOrQueryDemo demo = new SpanOrQueryDemo();
		demo.index();
		demo.search();
	}
	...
	private void index() throws IOException {
		...
		doc.add(new TextField("text", "the quick brown fox jumps over the lazy sick dog", Field.Store.YES));
	...
	private void search() throws IOException {
		...
		SpanQuery spanOrQuery = new SpanOrQuery(spanNotQuery, spanTermQuery);

		TopDocs topDocs = searcher.search(spanOrQuery, 100);
