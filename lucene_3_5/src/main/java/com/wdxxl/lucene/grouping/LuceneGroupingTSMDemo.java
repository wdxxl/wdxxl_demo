...
public class LuceneGroupingTSMDemo {
	private static Directory directory;
	private IndexReader reader;

	static {
		try {
			directory = FSDirectory.open(new File(System.getProperty("user.dir") + "/lib/lucene35/pinpoint_server_tsm"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public static void main(String[] args) {
		LuceneGroupingTSMDemo luceneGrouping = new LuceneGroupingTSMDemo();
	...
	private void searchGrouping() throws IOException, ParseException {
		BooleanQuery query = new BooleanQuery();
		QueryParser queryParser = new QueryParser(Version.LUCENE_35, "pps_tsm_message", new KeywordAnalyzer());
		Query query2 = queryParser.parse("eng");
		query.add(query2, Occur.MUST);
		query.add(new TermQuery(new Term("pps_tsm_library_id", "81443bba-d7d8-464f-bb6a-9eb47fba77db")), Occur.MUST);
		query.add(new TermQuery(new Term("pps_tsm_type", "fault")), Occur.MUST);

		// 搜索相似度最高的3条记录并且分组
		int topNGroups = 10; // 前N条中分组 去几个分组
		...
		TermFirstPassGroupingCollector c1 = new TermFirstPassGroupingCollector("pps_tsm_code", groupSort,
		...
		TermSecondPassGroupingCollector c2 = new TermSecondPassGroupingCollector("pps_tsm_code", topGroups, groupSort,
		...
		if (requiredTotalGroupCount) {
			allGroupsCollector = new TermAllGroupsCollector("pps_tsm_code");
		...
		for (GroupDocs<String> groupDocs : groupsResult.groups) {
			...
			for (ScoreDoc scoreDoc : groupDocs.scoreDocs) {
				...
				System.out.println("group[" + groupIdx + "][" + docIdx + "]{docId:pps_tsm_message}: " + doc.get("pps_tsm_message") +" : "+ doc.get("pps_tsm_code"));
			}
			System.out.println("***********************************************");
		}
	}
	...
	public void index() {
		...
		try {
			...
			doc.add(new Field("author", "author 1 2", Field.Store.YES, Field.Index.NOT_ANALYZED));
			doc.add(new Field("content", "random text", Field.Store.YES, Field.Index.ANALYZED));
			doc.add(new Field("id", "1", Field.Store.YES, Field.Index.NOT_ANALYZED));
			writer.addDocument(doc);

			doc = new Document();
			doc.add(new Field("author", "author 1 3", Field.Store.YES, Field.Index.NOT_ANALYZED));
			...
			doc = new Document();
			doc.add(new Field("author", "author21", Field.Store.YES, Field.Index.NOT_ANALYZED));
			...
			doc = new Document();
			doc.add(new Field("author", "author33", Field.Store.YES, Field.Index.NOT_ANALYZED));
			doc.add(new Field("content", "some more random text", Field.Store.YES, Field.Index.ANALYZED));
